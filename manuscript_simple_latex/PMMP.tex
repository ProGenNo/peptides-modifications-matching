...
\comment{}{Do we miss one of Harald's affiliations?}
...
In this work the modification site localization problem is addressed by a graph approach. For each PSM, a weighted  bipartite graph ($G = \{V,E,w\}$) is used to model all possible combinations of modifications on the amino acid chain. In this graph we determine two kinds of vertices, the ones that represent the different modifications (i.e. $D = \{d_1, d_2, \dots, d_k\}$) and the ones that represent all their possible acceptor sites (i.e. $A = \{a_1, a_2, \dots, a_n\}$). \comment{ For example, if for a modified peptide there are reported four acetylations and one dimethylation and there are 8 sites in the amino acid chain where either one or both of these modifications can occur, then the model will have 13 vertices in total ($\|D\| = 5$ and $\|A\| = 8$).}{I added this example here to make the model more clear, is it helping?} The set of vertices $V$ is then formed by the union of the sets $A$ and $D$ (i.e. $V = A \cup D$). For each modification, an edge is formed between the corresponding vertex and the vertex of each one of its possible acceptor sites. A weight $w_{i,j}$ is assigned to each edge $e_{i,j}$ corresponding to the localization score of the modification $d_i$ to the acceptor site $a_j$.
...
\comment{}{Since for the implementations both for python and java we are transforming the proposed model a bit (because the available libraries don't solve the exact same algorithm we need), should I describe this transformation more mathematically so it's more solid as an explanation? (because the algorithm freak in my head is not very happy with this description I have now) But this would make the text a bit more difficult to read from the perspective of a bioinformatician user of the tool, and also I don't know if such an extensive explanation is relevant for such papers, so which is the best approach in our case? Should I just leave the text as it is now? }
...
\subsection{Histone modification localization example}
 ...
 \comment{}{\textbf{Figure 1:} Should we say that the edges with the black localization scores are the ones chosen by the algorithm to give the best matching? Also please check my comment on the Figure 1 legend.}
...
\comment{TODO}{A word on conflict resolution and \textbf{"corner cases"}? \textbf{*} The corner case where a low score needs to be prioritized is solved by ensuring the maximum cardinality of the matching, as explained in the Model section. Should I have a relevant example to explain it further here, with a graph like Figure 1 ? \textbf{*}}
